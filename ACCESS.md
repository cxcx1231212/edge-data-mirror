...
参数值应经过 URL 编码。成功后服务端签发有效期 24 小时的 HttpOnly、Secure、SameSite=Lax Cookie；浏览器把地址改成 `/index.html`，清除地址栏中的参数。页面跳转和同站接口随后使用 Cookie，无须在每个链接中重复 t。Cookie 过期后重新使用带参数入口。更换 Secret 会立即使旧会话失效。
