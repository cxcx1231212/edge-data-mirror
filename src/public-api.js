...
async function memberPosts(url,db){
  ...
  if(id){const post=await db.prepare("SELECT id,lottery_type,period,source_name AS title,content,COALESCE(NULLIF(result_special,''),'yixiao') AS board,hit_status,created_at,updated_at FROM materials WHERE id=? AND section_key='member' AND published=1 LIMIT 1").bind(id).first();return post?json({success:true,post}):json({success:false,message:"会员贴不存在"},404);}
  const limit=Math.min(50,Math.max(1,Number(url.searchParams.get("limit")||20))),{results=[]}=await db.prepare("SELECT id,lottery_type,period,source_name AS title,content,COALESCE(NULLIF(result_special,''),'yixiao') AS board,hit_status,created_at,updated_at FROM materials WHERE section_key='member' AND published=1 ORDER BY id DESC LIMIT ?").bind(limit).all();
