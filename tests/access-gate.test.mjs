...
test('four entry cases, isolated navigation, cookie, no token in response',async()=>{
  ...
  const valid=await run('/?t='+encodeURIComponent(secret));assert.equal(valid.status,200);assert.match(await valid.text(),/page-loader/);
...
test('HTML, aliases, JSON, APIs and member snapshots require session',async()=>{
  ...
  assert.equal((await run('/index.html',{cookie})).status,200);
