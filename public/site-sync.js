(function () {
  ...
  function loadMemberPosts(){
    const panel=document.querySelector('.section .more[href="yixiao.html"]')?.closest('.section');
    if(!panel)return Promise.resolve();
    panel.querySelector('.member-post-list')?.remove();
    if(!document.getElementById('member-post-styles')){
    ...
    return fetch('/api/member-posts?limit=10',{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject()).then(payload=>{
      if(!payload?.success||!Array.isArray(payload.posts)||!payload.posts.length)return;
      const list=document.createElement('div');list.className='member-post-list';
      payload.posts.forEach(post=>{if(!post?.id||!post?.title)return;const link=document.createElement('a');link.className='member-post-link';link.href='/yixiao-member-preview.html?id='+encodeURIComponent(post.id);const title=document.createElement('span');title.className='member-post-title';title.textContent=post.title;link.appendChild(title);list.appendChild(link);});
      const grid=panel.querySelector('.grid-3');if(grid&&list.childElementCount)grid.before(list);
    }).catch(()=>{});
  }
  function loadMemberPostsNative(){
    document.querySelectorAll('.member-post-entry,.member-post-list').forEach(node=>node.remove());
    return fetch('/api/member-posts?limit=50',{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject()).then(payload=>{if(!payload?.success||!Array.isArray(payload.posts))return;const groups={},seen=new Set(),displayTitle=post=>{const raw=String(post.title||'').trim();if(/^\d+期[:：]/.test(raw)&&raw.includes('【'))return raw;const modern=String(post.period||'').startsWith('member-v2:'),kind=modern?String(post.period).slice(10).replace(/^【|】$/g,''):(String(post.content||'').match(/《[^》]+》\s*([^【\r\n]+)/)?.[1]||categoryNames[post.board]||'平特一肖').trim(),period=activeMaterialPeriod||String(document.querySelector('#apiPeriod,.period,.issue')?.textContent||'').match(/\d+/)?.[0]||'',history=Math.max(0,Number(post.hit_status)||0);return (period?period+'期: ':'')+raw+'→【'+kind+'】'+(history?'【'+history+'期中'+history+'期】':'')};payload.posts.forEach(post=>{if(!post?.id||!post?.title)return;const board=sectionKeys.includes(post.board)?post.board:'yixiao',key=Number(post.lottery_type||currentType())+'|'+board+'|'+String(post.title).trim();if(Number(post.lottery_type||currentType())!==currentType()||seen.has(key))return;seen.add(key);post.displayTitle=displayTitle(post);(groups[board]??=[]).push(post)});Object.entries(groups).forEach(([board,posts])=>{const panel=document.querySelector('.section .more[href="'+board+'.html"]')?.closest('.section'),grid=panel?.querySelector('.grid-3');if(!grid)return;const fragment=document.createDocumentFragment();posts.forEach(post=>{const card=document.createElement('div');card.className='material title-only member-post-entry';card.setAttribute('role','link');card.tabIndex=0;card.setAttribute('aria-label','查看 '+post.displayTitle);const name=document.createElement('div');name.className='name';name.textContent=post.displayTitle;const pick=document.createElement('div');pick.className='pick';const open=()=>location.href='/yixiao-member-preview.html?id='+encodeURIComponent(post.id);card.onclick=open;card.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open();}};card.append(name,pick);fragment.appendChild(card)});grid.prepend(fragment)});}).catch(()=>{});
  }
  ...
  function renderDetail(section) {
    ...
    api(section).then(async data => {
      ...
      const displayRecords = [...records].sort((a,b)=>Number(a.period||0)-Number(b.period||0));
  ...
  function ensureAdStyles(){
    ...
    style.textContent+='.banner-ad img{height:auto;max-height:none;aspect-ratio:auto;object-fit:contain}';
    ...
    style.textContent+='.shared-detail-footer{margin:14px 0 4px;padding:18px 12px;text-align:center;line-height:1.9;color:#f6cb51;border-top:1px solid #332a19}.shared-detail-footer a{color:#ff4b4b;text-decoration:none;font-weight:900}';
  ...
  function loadSharedFooter(anchor){
    fetch('/api/shared-footer',{cache:'no-store'}).then(r=>r.ok?r.json():Promise.reject()).then(payload=>{if(!payload?.success||!payload.html)return;let box=document.querySelector('.shared-detail-footer');if(!box){box=document.createElement('div');box.className='shared-detail-footer';anchor.before(box)}box.innerHTML=payload.html}).catch(()=>{});
  }
  function boot() {
    ensureThreeStyles();
    ...
    if(list){const ad=makeAd(match[2]?'detail':'list',match[2]?'内容页广告位':'列表页广告位');match[2]?list.after(ad):document.querySelector('.topbar')?.after(ad);if(match[2])loadSharedFooter(ad);loadAds();}
    track(match[1]);
