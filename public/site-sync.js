(function () {
  ...
  function renderHomepage() {
    ...
    return fetch('/api/member-posts?limit=50&lotteryType='+encodeURIComponent(currentType()),{cache:'no-store'}).then(response=>response.ok?response.json():Promise.reject()).then(payload=>{document.querySelectorAll('.member-post-entry,.member-post-list').forEach(node=>node.remove());if(!payload?.success||!Array.isArray(payload.posts))return;const groups={};payload.posts.forEach(post=>{if(!post?.id||!post?.title)return;const board=sectionKeys.includes(post.board)?post.board:'yixiao';(groups[board]??=[]).push(post)});Object.entries(groups).forEach(([board,posts])=>{const target=document.querySelector('.section .more[href="'+board+'.html"]')?.closest('.section'),grid=target?.querySelector('.grid-3');if(!grid)return;const fragment=document.createDocumentFragment();posts.forEach(post=>{const card=document.createElement('div');card.className='material title-only member-post-entry';card.setAttribute('role','link');card.tabIndex=0;card.setAttribute('aria-label','查看 '+post.title);const name=document.createElement('div');name.className='name';name.textContent=post.title;const pick=document.createElement('div');pick.className='pick';const open=()=>location.href='/yixiao-member-preview.html?id='+encodeURIComponent(post.id);card.onclick=open;card.onkeydown=event=>{if(event.key==='Enter'||event.key===' '){event.preventDefault();open();}};card.append(name,pick);fragment.appendChild(card)});grid.prepend(fragment)});}).catch(()=>{});
  }
